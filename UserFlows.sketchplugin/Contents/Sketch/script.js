...
var defineLink = function (context) {
  ...
  if (selection.count() != 2) {
    ...
    if (
      ...
      selection.lastObject().className() == "MSArtboardGroup" ||
      selection.lastObject().className() == "MSSymbolMaster"
    ) {
    ...
    if (
      !destArtboard ||
      linkLayer.className() == "MSArtboardGroup" ||
      linkLayer.className() == "MSSymbolMaster" ||
      linkLayer.parentArtboard() == destArtboard
    ) {
      validSelection = false;
    }
  }
  ...
  if (!validSelection) {
    showAlert(
      strings["defineLink-invalidSelectionTitle"],
      strings["defineLink-invalidSelectionMessage"]
    );
    return;
  }
  ...
  if (context.command.identifier() == "defineLinkWithOptions") {
    if (sketchVersion < sketchVersion51) {
      showAlert(
        "Not Supported",
        "This feature is only available in Sketch 51 and above."
      );
      return;
    }
    ...
    if (runModalForWindow(settingsWindow) == "1000") {
      ...
      if (shareableObjectRef) {
        ...
        if (!localObject && sketchVersion >= sketchVersion52) {
          localObject = AppController.sharedInstance()
            .librariesController()
      ...
      lastUsedForeignStyleID =
        shareableObjectRef && shareableObjectRef.sourceLibrary()
          ? shareableObjectRef.sharedObjectID()
          : nil;
      context.command.setValue_forKey_onDocument_forPluginIdentifier(
        lastUsedForeignStyleID,
        "lastUsedForeignStyleID",
  ...
  var artboardID = context.command.valueForKey_onLayer_forPluginIdentifier(
    "artboardID",
    destArtboard,
    kPluginDomain
  );
  ...
  var showingConnections =
    NSUserDefaults.standardUserDefaults().objectForKey(kShowConnectionsKey) ||
    1;
  ...
  if (showingConnections == 1) {
    ...
    doc.showMessage(
      strings["defineLink-linkDefined"] +
        " " +
        linkLayer.name() +
        " → " +
        destArtboard.name()
...
var stylesDropdownForContext_selectedStyleID_sharedObjectID = function (
  context,
  selectedStyleID,
  sharedObjectID
) {
  ...
  var docStyles = context.document
    .documentData()
    .layerStyles()
    .sharedStyles()
  ...
  var docStylesTitle =
    docStyles.count() > 0 ? "Document Styles" : "No Document Styles";
  menuItem = NSMenuItem.alloc().initWithTitle_action_keyEquivalent(
    docStylesTitle,
    nil,
    ""
  ...
  var libs = AppController.sharedInstance()
    .librariesController()
    .availableLibraries();
  var lib, libStyles;
    ...
    if (libStyles.count() > 0) {
      ...
      while ((sharedStyle = stylesLoop.nextObject())) {
        ...
        if (
          !selectedItem &&
          sharedObjectID &&
          sharedStyle.objectID() == sharedObjectID
        ) {
          selectedItem = menuItem;
        }
        stylesDropdown.menu().addItem(menuItem);
...
var editConnectionStyle = function (context) {
  ...
  if (numLayers == 0) {
    showAlert(
      strings["defineLink-invalidSelectionTitle"],
      strings["editLinkOptions-invalidSelectionMessage"]
    );
    return;
  }
  ...
  if (runModalForWindow(settingsWindow) == "1000") {
    ...
    var linkLayer,
      foreignStyleID = shareableObjectRef
        ? shareableObjectRef.sharedObjectID()
        : nil,
  ...
  if (showingConnections == 1) {
    ...
    var message =
      context.selection.count() == 1
        ? strings["removeLink-linkRemoved"]
        : strings["removeLink-linksRemoved"];
...
var removeAllLinks = function (context) {
  ...
  if (response == "1000") {
    ...
    var layersInScope =
      selectedOptionID == 1
        ? context.selection.valueForKeyPath(
            "@distinctUnionOfArrays.parentArtboard.children"
          )
        : doc.currentPage().children();

    var linkLayersPredicate = NSPredicate.predicateWithFormat(
        "userInfo != nil && function(userInfo, 'valueForKeyPath:', %@).destinationArtboardID != nil",
        kPluginDomain
      ),
...
var relinkArtboardsAfterCopy = function (context) {
  var showsRelinkWarning =
    NSUserDefaults.standardUserDefaults().objectForKey(kRelinkWarningKey) || 1;
...
var confirmRelinkArtboards = function (context) {
  var destinationArtboards = context.document
      .currentPage()
      .artboards()
      .filteredArrayUsingPredicate(
        NSPredicate.predicateWithFormat(
          "userInfo != nil && function(userInfo, 'valueForKeyPath:', %@).artboardID != nil",
          kPluginDomain
        )
      ),
    loop = destinationArtboards.objectEnumerator(),
    validArtboardIDs = context.document
      .currentPage()
      .valueForKeyPath("artboards.@unionOfObjects.objectID"),
    relinkCount = 0,
    artboard,
    cachedID,
    artboardID,
  ...
  while ((artboard = loop.nextObject())) {
    ...
    cachedID = context.command.valueForKey_onLayer_forPluginIdentifier(
      "artboardID",
      artboard,
      kPluginDomain
    );
    ...
    if (artboardID != cachedID && !validArtboardIDs.containsObject(cachedID)) {
      linkLayers = context.document
        .currentPage()
        .children()
        .filteredArrayUsingPredicate(
          NSPredicate.predicateWithFormat(
            "userInfo != nil && function(userInfo, 'valueForKeyPath:', %@).destinationArtboardID == %@",
            kPluginDomain,
            cachedID
          )
        );
  ...
  if (showingConnections == 1) {
    ...
    var infoText = strings[
      "relinkArtboards-relinkComplete"
...
var editConditionsForArtboard = function (
  currentArtboard,
  context,
  forceNewCondition
) {
  ...
  var artboardIsConditional =
      forceNewCondition == true
        ? 0
        : context.command.valueForKey_onLayer_forPluginIdentifier(
            kConditionalArtboardKey,
            currentArtboard,
            kPluginDomain
          ) || 0,
    hasElse = artboardIsConditional
      ? context.command.valueForKey_onLayer_forPluginIdentifier(
          "hasElse",
          currentArtboard,
          kPluginDomain
  ...
  if (artboardIsConditional == 1) {
    var predicate = NSPredicate.predicateWithFormat(
        "userInfo != nil && function(userInfo, 'valueForKeyPath:', %@).isCondition != nil",
        kPluginDomain
      ),
      conditionLayers = currentArtboard
        .children()
        .filteredArrayUsingPredicate(predicate);
    ...
    if (conditionLayers.count() != 0) {
      ...
      while ((conditionLayer = loop.nextObject())) {
        ...
        conditionField = NSTextField.alloc().initWithFrame(
          NSMakeRect(21, 0, 250, 22)
        );
    ...
    predicate = NSPredicate.predicateWithFormat(
      "userInfo != nil && function(userInfo, 'valueForKeyPath:', %@).isElse != nil",
      kPluginDomain
    );
    var elseLabel = currentArtboard
      .children()
      .filteredArrayUsingPredicate(predicate)
      .firstObject();
    if (elseLabel) {
  ...
  conditionField.setPlaceholderString(
    strings["addCondition-ifConditionPlaceholder"]
  );
  conditionView.addSubview(conditionField);
  settingsWindow.addAccessoryView(conditionView);
  conditionFields.push(conditionField);
  ...
  if (response != "1001") {
    ...
    if (artboardIsConditional) {
      ...
      conditionBoard.setName(
        "-" + strings["addCondition-conditionArtboardName"] + "-"
      );
    ...
    for (var i = 0; i < numConditions; i++) {
      ...
      conditionLabel
        .frame()
        .setWidth(conditionBoardWidth - (conditionSpacing + 8) * 2);
      conditionLabel.setTextBehaviour(1);
      conditionLabel.setStringValue(conditionValue);
      conditionLabel.addAttribute_value(
        NSFontAttributeName,
        NSFont.fontWithName_size("HelveticaNeue", conditionFontSize)
      );
      ...
      if (sketchVersion < sketchVersion52) {
        conditionBox = MSShapeGroup.shapeWithPath(
          MSRectangleShape.alloc().initWithFrame(
            NSMakeRect(
              conditionSpacing,
              listY,
              conditionBoardWidth - conditionSpacing * 2,
              conditionBoxHeight
            )
          )
        );
        ...
        conditionBox = MSRectangleShape.alloc().initWithFrame(
          NSMakeRect(
            0,
            0,
            conditionBoardWidth - conditionSpacing * 2,
      ...
      conditionBox
        ...
        .setColor(
          MSImmutableColor.colorWithSVGString("#f9f9f9").newMutableCounterpart()
        );
      ...
      conditionGroup =
        sketchVersion < sketchVersion52
          ...
          : MSLayerGroup.groupWithLayers(layersArray);
      ...
      conditionGroup.setName(
        strings["addCondition-conditionGroupName"] + " " + count
      );
    ...
    if (artboardIsConditional != 1) {
      var currentView =
        sketchVersion < sketchVersion48
          ? context.document.currentView()
          : context.document.contentDrawView();
      var vcr = currentView.visibleContentRect(),
        absPosition = NSMakePoint(
          CGRectGetMidX(vcr) -
            CGRectGetMidX(conditionBoard.absoluteRect().rect()),
          CGRectGetMidY(vcr) -
            CGRectGetMidY(conditionBoard.absoluteRect().rect())
        );
...
var addCondition = function (context) {
  ...
  var parentArtboards = context.selection.valueForKeyPath(
      "@distinctUnionOfObjects.parentArtboard"
    ),
    currentArtboard = parentArtboards.firstObject();

  editConditionsForArtboard(
    currentArtboard,
    context,
    parentArtboards.count() != 1
  );
};
...
var gotoDestinationArtboard = function (context) {
  ...
  if (!validSelection) {
    showAlert(
      strings["gotoArtboard-invalidSelectionTitle"],
      strings["gotoArtboard-invalidSelectionMessage"]
    );
    return;
  }
  ...
  var doc = context.document,
    destinationArtboard = doc
      ...
      .artboards()
      .filteredArrayUsingPredicate(
        NSPredicate.predicateWithFormat("objectID == %@", destinationArtboardID)
      )
      .firstObject();
  if (destinationArtboard) {
    ...
    var cRect = currentView.visibleContentRect(),
      ...
      rects = context.command.valueForKey_onDocument_forPluginIdentifier(
        "contentRectsHistory",
        doc.documentData(),
        kPluginDomain
      );
...
var goBackToLink = function (context) {
  ...
  var doc = context.document,
    rects = context.command
      .valueForKey_onDocument_forPluginIdentifier(
        "contentRectsHistory",
        doc.documentData(),
        kPluginDomain
      )
...
var recursivelyDetachSymbolInstance = function (instance) {
  ...
  var loop = group.children().objectEnumerator(),
    layer;
...
var artboardWithDetachedSymbolsFromArtboard = function (artboard) {
  ...
  var symbolInstances = detachedArtboard
    .children()
    .filteredArrayUsingPredicate(
      NSPredicate.predicateWithFormat("className == 'MSSymbolInstance'")
    );
  var loop = symbolInstances.objectEnumerator(),
    instance;
  ...
  while ((artboard = loop.nextObject())) {
    if (
      artboard
        .ancestry()
        .layer()
        .containsFlowWithSymbolsFromDocument(doc.immutableDocumentData())
    ) {
...
var generateFlow = function (context) {
  ...
  var linkLayers = currentPage
    .children()
    .filteredArrayUsingPredicate(linkLayerPredicate);

  if (linkLayers.count() == 0 && !pageContainsFlowConnections) {
    showAlert(
      strings["generateFlow-noLinksTitle"],
      strings["generateFlow-noLinksMessage"]
    );
    return;
  }

  var artboardsWithLinks = linkLayers.count()
    ? linkLayers.valueForKeyPath("@distinctUnionOfObjects.parentArtboard")
    : NSArray.array();

  if (pageContainsFlowConnections) {
    artboardsWithLinks = NSSet.setWithArray(
      artboardsWithLinks.arrayByAddingObjectsFromArray(
        getArtboardsWithFlowConnectionsInPage(currentPage, doc)
      )
    ).allObjects();
  }
  ...
  while ((artboardWithLinks = loop.nextObject())) {
    artboardTitle = artboardWithLinks.isFlowHome()
      ? "⚑ " + artboardWithLinks.name()
      : artboardWithLinks.name();
    menuItem = NSMenuItem.alloc().initWithTitle_action_keyEquivalent(
      artboardTitle,
      nil,
  ...
  var homeScreenID = context.command.valueForKey_onLayer_forPluginIdentifier(
    "homeScreenID",
    currentPage,
    kPluginDomain
  );
  if (homeScreenID) {
    var artboardIDs = artboardsWithLinks.valueForKeyPath(
      "@unionOfObjects.objectID"
    );
  ...
  if (response == "1000") {
    ...
    var initialArtboard = artboardsWithLinks.objectAtIndex(
      artboardsDropdown.indexOfSelectedItem()
    );
    generateFlowWithSettings(context, settings, initialArtboard, currentPage);
  }
};

var generateFlowWithSettings = function (
  context,
  settings,
  initialArtboard,
  sourcePage,
  ...
  var doc = context.document;
  var connectionsOverlayPredicate = NSPredicate.predicateWithFormat(
      "userInfo != nil && function(userInfo, 'valueForKeyPath:', %@).isConnectionsContainer == true",
      kPluginDomain
    ),
    connectionsOverlay = sourcePage
      .children()
      .filteredArrayUsingPredicate(connectionsOverlayPredicate)
      .firstObject(),
  ...
  var exportScale = 1,
    ...
    secondaryTextColor,
    flowBackgroundColor,
    artboardIsConditional,
    isCondition,
    destinationArtboardIsConditional,
    linkRect,
    destinationRect,
    sharedBorderStyleID;

  context.command.setValue_forKey_onLayer_forPluginIdentifier(
    initialArtboard.objectID(),
    "homeScreenID",
    sourcePage,
    kPluginDomain
  );
  ...
  screenShadowColor.setAlpha(0.2);
  ...
  while (artboardsToExport.length) {
    ...
    exportURL = tempFolderURL
      .URLByAppendingPathComponent(artboard.objectID())
      .URLByAppendingPathExtension(exportFormat);
    ...
    if (sketchVersion < sketchVersion51) {
      ...
      screenLayer = MSBitmapLayer.alloc().initWithFrame_image(
        NSZeroRect,
        screenImageData
      );
    }
    ...
    if (artboardIsConditional == 0) {
      ...
      artboardNameLabel
        .absoluteRect()
        .setY(
          artboard.absoluteRect().y() -
            artboardNameLabel.absoluteRect().height() / exportScale -
            6 * exportScale
        );
    ...
    linkLayers = artboard
      ...
      .sortedArrayUsingDescriptors([
        NSSortDescriptor.sortDescriptorWithKey_ascending(
          "absoluteRect.y",
          true
        ),
      ]);
    ...
    for (var i = 0; i < linkLayersCount; i++) {
      ...
      if (destinationArtboard) {
        ...
        linkRect =
          linkLayer.parentArtboard() == nil
            ? linkLayer.absoluteRect().rect()
            : CGRectIntersection(
                linkLayer.absoluteRect().rect(),
    ...
    if (
      ...
      sketchVersion >= sketchVersion49
    ) {
      var immutableArtboard = artboard.ancestry().layer();
      if (
        immutableArtboard.containsFlowWithSymbolsFromDocument(
          doc.immutableDocumentData()
        )
      ) {
        detachedArtboard = artboardWithDetachedSymbolsFromArtboard(artboard);

        var flowConnections = detachedArtboard.valueForKeyPath(
          "children.@distinctUnionOfObjects.flow"
        );
        var loop = flowConnections.objectEnumerator(),
          flowConnection,
          dropPointX,
          dropPointY;
        ...
        while ((flowConnection = loop.nextObject())) {
          ...
          if (flowConnection.isBackAction()) {
            ...
            connection.destinationRect = CGRectMake(
              dropPointX - 20,
              dropPointY,
              10,
              10
            ...
            connection.linkIsCrossPage =
              destinationArtboard.parentPage() != sourcePage;

            if (connection.linkIsCrossPage) {
              dropPointX =
                artboard.absoluteRect().x() +
                artboard.absoluteRect().width() +
                50 * exportScale;
              dropPointY = artboard.absoluteRect().y() - 30 * exportScale;
              ...
              connection.artboardParentName = destinationArtboard
                .parentPage()
                .name();
            } else {
              dropPointX =
                destinationArtboard.absoluteRect().x() -
                dropPointOffset * exportScale;
              dropPointY =
                destinationArtboard.absoluteRect().y() +
                destinationArtboard.absoluteRect().height() / 2;
  ...
  var newGroup =
    sketchVersion < sketchVersion52
      ...
      : MSLayerGroup.groupWithLayers(layers);
  ...
  flowNameLabel.addAttribute_value(
    NSFontAttributeName,
    NSFont.fontWithName_size("HelveticaNeue", 36 * exportScale)
  );
  ...
  if (flowDescription && flowDescription != "") {
    ...
    flowDescriptionLabel
      .frame()
      .setWidth(Math.min(groupBounds.size.width, 600));
    flowDescriptionLabel.setTextBehaviour(1);
    flowDescriptionLabel.setStringValue(flowDescription);
    flowDescriptionLabel.addAttribute_value(
      NSFontAttributeName,
      NSFont.fontWithName_size("HelveticaNeue", 16 * exportScale)
    );
    ...
    yPos =
      flowDescriptionLabel.frame().y() + flowDescriptionLabel.frame().height();
  }
  ...
  if (showModifiedDate == 1) {
    ...
    if (modifiedBy && modifiedBy != "") {
      modifiedOnText = strings["generateFlow-modifiedBy"]
        .stringByReplacingOccurrencesOfString_withString(
          "%date%",
          formatter.stringFromDate(NSDate.date())
        )
        .stringByReplacingOccurrencesOfString_withString("%user%", modifiedBy);
    } else {
      modifiedOnText = strings[
        "generateFlow-modifiedOnDate"
      ].stringByReplacingOccurrencesOfString_withString(
        "%date%",
        formatter.stringFromDate(NSDate.date())
      );
    ...
    modifiedDateLabel
      .frame()
      .setWidth(Math.min(groupBounds.size.width - outerPadding * 2, 600));
    modifiedDateLabel.setTextBehaviour(1);
    modifiedDateLabel.setStringValue(modifiedOnText);
    modifiedDateLabel.addAttribute_value(
      NSFontAttributeName,
      NSFont.fontWithName_size("HelveticaNeue", 12 * exportScale)
    );
  ...
  var flowPageName = settings.flowPageName,
    flowPage = doc
      .pages()
      .filteredArrayUsingPredicate(
        NSPredicate.predicateWithFormat("name == %@", flowPageName)
      )
      .firstObject();
  if (!flowPage) {
  ...
  if (shouldOrganize && flowPageName == "_Flows") {
    ...
    if (flowPage.originForNewArtboard) {
      ...
      originForNewArtboard = flowPage.originForNewArtboardWithSize(
        flowBoard.absoluteRect().rect().size
      );
    }
  ...
  var visibleContentRect = NSInsetRect(
    flowBoard.absoluteRect().rect(),
    -60,
    -60
  );
  var currentView =
    sketchVersion < sketchVersion48 ? doc.currentView() : doc.contentDrawView();
...
var updateFlow = function (context) {
  ...
  var sourcePageID = context.command.valueForKey_onLayer_forPluginIdentifier(
    "pageID",
    currentArtboard,
    kPluginDomain
  );
  if (!homeScreenID || !sourcePageID) {
    showAlert(
      strings["updateFlow-noFlowArtboard"],
      strings["updateFlow-noFlowArtboardMessage"]
    );
    return;
  }
  ...
  if (!sourcePage || !initialArtboard) {
    showAlert(
      strings["updateFlow-noInitialArtboard"],
      strings["updateFlow-noInitialArtboardMessage"]
    );
    return;
  }
  ...
  var titleLabelID = context.command.valueForKey_onLayer_forPluginIdentifier(
    "titleLayerID",
    currentArtboard,
    kPluginDomain
  );
...
var sharedLayerStylesForContext = function (context) {
  ...
  var doc = context.document || context.actionContext.document,
    ...
    foreignStyles = doc
      .documentData()
      .valueForKeyPath("foreignLayerStyles.@unionOfObjects.localSharedStyle"),
    availableStyles = localStyles.arrayByAddingObjectsFromArray(foreignStyles),
    predicate = NSPredicate.predicateWithFormat(
      "style.firstEnabledFill == nil"
    ),
...
var redrawConnections = function (context) {
  ...
  var linkLayersPredicate = NSPredicate.predicateWithFormat(
    ...
    linkLayers = doc
      .currentPage()
      .children()
      .filteredArrayUsingPredicate(linkLayersPredicate),
    loop = linkLayers.objectEnumerator(),
    connections = [],
    dropPointOffset = sketchVersion < sketchVersion51 ? 10 : 4,
    linkLayer,
  ...
  var parentArtboards = linkLayers.valueForKeyPath(
      "@distinctUnionOfObjects.parentArtboard"
    ),
...
var drawConnections = function (
  ...
  var connectionsCount = connections.length,
    ...
    controlPoint2,
    controlPoint1Offset,
    controlPoint2OffsetX,
    controlPoint2OffsetY,
    linePath,
    lineLayer,
    destinationArtboardIsConditional,
    originPoint,
    ...
    connectionPosition,
    controlPointOffset,
    minControlPointOffset,
    sharedBorderStyleID,
    sharedBorderStyle,
    linkLayerHasSharedStyleReference,
    straightConnection;
  ...
  for (var i = 0; i < connectionsCount; i++) {
    ...
    if (showLinkRects == 1 && connection.linkIsCondition != 1) {
      if (linkRect.size.width < minimumTapArea) {
        linkRect = NSInsetRect(
          linkRect,
          (linkRect.size.width - minimumTapArea) / 2,
          0
        );
      }
      if (linkRect.size.height < minimumTapArea) {
        linkRect = NSInsetRect(
          linkRect,
          0,
          (linkRect.size.height - minimumTapArea) / 2
        );
      }
      ...
      if (sketchVersion < sketchVersion50) {
        ...
        hitAreaLayer = MSShapeGroup.shapeWithBezierPath(
          MSPath.pathWithBezierPath(path)
        );
      } else {
    ...
    if (
      destinationRect &&
      (magnetsType == "nsew" ||
        connection.isBackAction ||
        connection.linkIsCrossPage)
    ) {
      destinationRect = CGRectInset(
        destinationRect,
        destinationRectInset,
        destinationRectInset
      );
      ...
      minControlPointOffset =
        connection.isBackAction || connection.linkIsCrossPage ? 0 : 160;

      switch (connectionPosition) {
        case "right":
          startPoint = NSMakePoint(
            CGRectGetMaxX(linkRect),
            CGRectGetMidY(linkRect)
          );
          dropPoint = NSMakePoint(
            CGRectGetMinX(destinationRect),
            CGRectGetMidY(destinationRect)
          );

          controlPointOffset = Math.max(
            Math.min(Math.abs(dropPoint.x - startPoint.x), 160),
            minControlPointOffset
          );
          controlPoint1 = NSMakePoint(
            startPoint.x + controlPointOffset,
            startPoint.y
          );
          controlPoint2 = NSMakePoint(
            dropPoint.x - controlPointOffset,
            dropPoint.y
          );
        ...
        case "left":
          startPoint = NSMakePoint(
            CGRectGetMinX(linkRect),
            CGRectGetMidY(linkRect)
          );
          dropPoint = NSMakePoint(
            CGRectGetMaxX(destinationRect),
            CGRectGetMidY(destinationRect)
          );
        ...
        case "top":
          startPoint = NSMakePoint(
            CGRectGetMidX(linkRect),
            CGRectGetMinY(linkRect)
          );
          dropPoint = NSMakePoint(
            CGRectGetMidX(destinationRect),
            CGRectGetMaxY(destinationRect)
          );

          controlPointOffset = Math.max(
            Math.min(Math.abs(dropPoint.y - startPoint.y), 160),
            minControlPointOffset
          );
          controlPoint1 = NSMakePoint(
            startPoint.x,
            startPoint.y - controlPointOffset
          );
          controlPoint2 = NSMakePoint(
            dropPoint.x,
            dropPoint.y + controlPointOffset
          );
        ...
        case "bottom":
          dropPoint = NSMakePoint(
            CGRectGetMidX(destinationRect),
            CGRectGetMinY(destinationRect)
          );
          startPoint = NSMakePoint(
            CGRectGetMidX(linkRect),
            CGRectGetMaxY(linkRect)
          );
      ...
      dropPoint = destinationArtboardIsConditional
        ? NSMakePoint(
            connection.dropPoint.x + 5 * exportScale,
            connection.dropPoint.y + 10 * exportScale
          )
        : NSMakePoint(connection.dropPoint.x, connection.dropPoint.y);
      if (dropPoint.x < CGRectGetMinX(linkRect)) {
        dropPoint = NSMakePoint(
          dropPoint.x + 18,
          dropPoint.y - dropPointYOffset / exportScale
        );
        arrowRotation = 90;
        arrowOffsetX = 2;
        if (dropPoint.y < CGRectGetMinY(linkRect)) {
          startPoint = NSMakePoint(
            CGRectGetMidX(linkRect),
            CGRectGetMinY(linkRect) + 5
          );
          controlPoint1Offset = Math.max(
            Math.abs(dropPoint.y - startPoint.y) / 2,
            200
          );
          controlPoint1 = NSMakePoint(
            startPoint.x,
            startPoint.y - controlPoint1Offset
          );
        } else {
          startPoint = NSMakePoint(
            CGRectGetMidX(linkRect),
            CGRectGetMaxY(linkRect) - 5
          );
          controlPoint1Offset = Math.max(
            Math.abs(dropPoint.y - startPoint.y) / 2,
            200
        ...
        startPoint = NSMakePoint(
          CGRectGetMaxX(linkRect) - 5,
          CGRectGetMidY(linkRect)
        );
        controlPoint1Offset = Math.max(
          Math.abs(dropPoint.x - startPoint.x) / 2,
          100
        );
        controlPoint1 = NSMakePoint(
          startPoint.x + controlPoint1Offset,
          startPoint.y
        );
        controlPoint2OffsetY = 0;
        controlPoint2OffsetX = Math.max(
          Math.abs(dropPoint.x - startPoint.x) / 2,
          100
        );
        arrowRotation = 0;
      ...
      controlPoint2 = NSMakePoint(
        dropPoint.x - controlPoint2OffsetX,
        dropPoint.y + controlPoint2OffsetY
      );
    }

    if (!shouldUseMarkers) {
      linkRect = NSInsetRect(
        NSMakeRect(startPoint.x, startPoint.y, 0, 0),
        -5,
        -5
      );
    ...
    straightConnection =
      ...
      Math.abs(startPoint.x - dropPoint.x) < 20 ||
      Math.abs(startPoint.y - dropPoint.y) < 20;
    if (straightConnection) {
      linePath.lineToPoint(dropPoint);
      originPoint = CGPointMake(
        dropPoint.x - startPoint.x,
        dropPoint.y - startPoint.y
      );
      degrees = Math.atan2(originPoint.y, originPoint.x) * (180.0 / Math.PI);
      arrowRotation = degrees > 0.0 ? degrees : 360.0 + degrees;
      if (arrowRotation < 110 || arrowRotation > 255) {
        arrowOffsetX = 2;
      }
    } else if (connectionType == "curved") {
      linePath.curveToPoint_controlPoint1_controlPoint2(
        dropPoint,
        controlPoint1,
        controlPoint2
      );
    ...
    if (sketchVersion < sketchVersion50) {
      ...
      lineLayer = MSShapeGroup.shapeWithBezierPath(
        MSPath.pathWithBezierPath(linePath)
      );
    } else {
      lineLayer = MSShapePathLayer.layerWithPath(
        MSPath.pathWithBezierPath(linePath)
      );
    }
    ...
    if (connection.isBackAction) {
      ...
      backLabel.addAttribute_value(
        NSFontAttributeName,
        NSFont.fontWithName_size("HelveticaNeue", 11 * exportScale)
      );
      ...
      var backBackgroundHeight =
        Math.ceil(backLabel.frame().height()) + padding;

      var backBG;
      if (sketchVersion < sketchVersion52) {
        backBG = MSShapeGroup.shapeWithPath(
          MSRectangleShape.alloc().initWithFrame(
            NSMakeRect(0, 0, backBackgroundWidth, backBackgroundHeight)
          )
        );
        backBG.firstLayer().setCornerRadiusFloat(5);
      } else {
        backBG = MSRectangleShape.alloc().initWithFrame(
          NSMakeRect(0, 0, backBackgroundWidth, backBackgroundHeight)
        );
      ...
      backBG
        .absoluteRect()
        .setY(dropPoint.y - Math.ceil(backBackgroundHeight / 2));

      backLabel
        .absoluteRect()
        .setX(dropPoint.x - backBackgroundWidth + padding / 2);
      ...
      pageNameLabel.addAttribute_value(
        NSFontAttributeName,
        NSFont.fontWithName_size("HelveticaNeue", 11 * exportScale)
      );
      ...
      var linkBackgroundWidth =
        Math.ceil(
          Math.max(
            pageNameLabel.frame().width(),
            artboardNameLabel.frame().width()
          )
        ) + padding;
      var linkBackgroundHeight =
        Math.ceil(
          pageNameLabel.frame().height() +
            3 +
            artboardNameLabel.frame().height()
        ) + padding;

      var linkBG;
      if (sketchVersion < sketchVersion52) {
        linkBG = MSShapeGroup.shapeWithPath(
          MSRectangleShape.alloc().initWithFrame(
            NSMakeRect(0, 0, linkBackgroundWidth, linkBackgroundHeight)
          )
        );
        linkBG.firstLayer().setCornerRadiusFloat(5);
      } else {
        linkBG = MSRectangleShape.alloc().initWithFrame(
          NSMakeRect(0, 0, linkBackgroundWidth, linkBackgroundHeight)
        );
      ...
      pageNameLabel
        .absoluteRect()
        .setY(linkBG.absoluteRect().y() + padding / 2);

      artboardNameLabel.absoluteRect().setX(dropPoint.x + padding / 2);
      artboardNameLabel
        .absoluteRect()
        .setY(
          pageNameLabel.absoluteRect().y() +
            pageNameLabel.absoluteRect().height() +
            3
        );
      ...
      if (!shouldUseMarkers) {
        ...
        path.moveToPoint(
          NSMakePoint(dropPoint.x + arrowSize * 0.6, dropPoint.y)
        );
        path.lineToPoint(
          NSMakePoint(dropPoint.x - arrowSize, dropPoint.y + arrowSize * 0.6)
        );
        path.lineToPoint(
          NSMakePoint(dropPoint.x - arrowSize * 0.6, dropPoint.y)
        );
        ...
        if (sketchVersion < sketchVersion50) {
          ...
          arrow = MSShapeGroup.shapeWithBezierPath(
            MSPath.pathWithBezierPath(path)
          );
        } else {
          arrow = MSShapePathLayer.layerWithPath(
            MSPath.pathWithBezierPath(path)
          );
        }
...
var applySettings = function (context) {
  var doc = context.document;
  var connectionsLayerPredicate = NSPredicate.predicateWithFormat(
    "userInfo != nil && function(userInfo, 'valueForKeyPath:', %@).isConnectionsContainer == true",
    kPluginDomain
  );
  var connectionsGroup = doc
    .currentPage()
    .children()
    .filteredArrayUsingPredicate(connectionsLayerPredicate)
    .firstObject();
...
var updateManifestForSettingsChange = function (
  context,
  autoUpdateConnections
) {
  var manifestPath = context.plugin
      .url()
      .URLByAppendingPathComponent("Contents")
      .URLByAppendingPathComponent("Sketch")
      .URLByAppendingPathComponent("manifest.json")
      .path(),
    manifest = NSJSONSerialization.JSONObjectWithData_options_error(
...
var editShortcuts = function (context) {
  ...
  for (var i = 0; i < commandsCount; i++) {
    command = commands[i];
    if (
      !validCommands.containsObject(command.identifier) ||
  ...
  if (response == "1000") {
    for (var i = 0; i < commandsCount; i++) {
      ...
      if (typeof shortcutField === "undefined") continue;
...
var checkForUpdates = function (context) {
  ...
  var json = NSJSONSerialization.JSONObjectWithData_options_error(
      NSData.dataWithContentsOfURL(
        NSURL.URLWithString("https://abynim.github.io/UserFlows/version.json")
      ),
      0,
      nil
    ),
  ...
  updateAlert.setMessageText(
    updateAvailable
      ? strings["checkUpdates-updateAvailableTitle"]
      : strings["checkUpdates-noUpdateAvailableTitle"]
  );
  if (updateAvailable) {
    var infoText = strings["checkUpdates-updateAvailableMessage"]
      .stringByReplacingOccurrencesOfString_withString(
        "%currentVersion%",
        currentVersion
      )
    ...
    updateAlert.setInformativeText(
      strings["checkUpdates-noUpdateAvailableMessage"]
    );
...
var editLanguage = function (context) {
  ...
  if (response == "1000") {
    ...
    var manifestPath = context.plugin
      ...
      commands = manifest.commands,
      commandsCount = commands.count(),
      stringsFilePath = context.plugin
        .urlForResourceNamed(localeID + ".plist")
        .path(),
...
var parseContext = function (context) {
  iconImage = NSImage.alloc().initByReferencingFile(
    context.plugin.urlForResourceNamed("icon-internal.png").path()
  );
...
var getVersionNumberFromString = function (versionString) {
  var versionNumber =
    versionString.stringByReplacingOccurrencesOfString_withString(".", "") + "";
...
var logEvent = function (event, props) {
  ...
  var fProps = {
    ...
    sketchVersion: NSBundle.mainBundle().objectForInfoDictionaryKey(
      "CFBundleShortVersionString"
    ),
    uuid: uuid,
    locale: localeID,
  ...
  if (props) {
    for (var key in props) fProps[key] = props[key];
  }
  ...
  var payload = {
    ...
    json = NSJSONSerialization.dataWithJSONObject_options_error(
      payload,
      0,
      nil
    ),
